...
class RandomProxy(object):
    ...
    def __init__(self, settings):
        self.chosen_proxy = ''
        # 从地址获取一个ip
        self.chosen_proxy = get_random_proxy()
    ...
    def process_response(self, request, response, spider):
        ...
        if response.status in [403, 400, 302] and 'proxy' in request.meta:
            logging.debug(f'Response status: {response.status} '
                          f'using proxy {request.meta["proxy"]} '
                          f'retrying request to {request.url}')
            self.chosen_proxy = get_random_proxy()  # 这个代理被403,302了 重新获取
            return request
        return response

    def process_exception(self, request, exception, spider):
        if 'proxy' not in request.meta:
            logging.debug("没代理错了,需要检查")
            return
        else:
            logging.debug("有代理也错了，把数据库的ip删掉")
            request.meta["exception"] = True
            logging.debug("重新获取ip")
            self.chosen_proxy = get_random_proxy()
            return request
